...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      ...
      <el-button size="mini" style="margin: 0px;" v-show="this.articles.length>0 && showRestore"
  ...
  export default{
    ...
    methods: {
      ...
      restoreMany(){
        var selItems = this.selItems;
        for (var i = 0; i < selItems.length; i++) {
          this.restoreData.push(selItems[i].id)
        }
        this.restoreToBlog(selItems[0].state)
      },
      //翻页
      ...
      restoreToBlog(){
        ...
        this.$confirm('将该文件还原到原处，是否继续？','提示',{
          ...
          _this.loading = true;
          putRequest('article/restoreMany', {aids: _this.restoreData}).then(resp=> {
            if (resp.status == 200) {
              var data = resp.data;
              _this.$message({type: data.status, message: data.msg});
              if (data.status == 'success') {
